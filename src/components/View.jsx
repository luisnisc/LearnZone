...
function View() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        "http://192.168.1.140:8000/clase/api/api/"
      );
      setEx(response.data);
      setLoading(false);
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://192.168.1.140:8000/clase/api/api/${id}/`);
  ...
  return (
    <div>
      ...
      <div className="float-right ">
        <button
          className="Icono hover:text-blue-700 mr-3.5 ml-3.5"
          onClick={() => setb(!b)}
        >
          <BsGridFill />
        </button>
      ...
      >
        {ex.map((item) => (
          <div
            key={item.id}
            id="card-examen"
            className="p-6 max-w-m  border border-gray-200 rounded-lg shadow  my-10 h-60 "
          >
            ...
            <div className="mt-24 text-center">
              <button
                className="focus:outline-none  mt-4 text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5  mb-2 "
